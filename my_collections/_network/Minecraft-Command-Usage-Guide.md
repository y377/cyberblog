...
### Minecraft Bedrock Edition（基岩版）

====[游戏规则配置]====

| 游戏规则                        | 设置值     | 说明                                  |
| --------------------------- | ------- | ----------------------------------- |
| `commandBlockOutput`        | `true`  | 启用命令方块输出                            |
| `doDaylightCycle`           | `true`  | 启用昼夜交替                              |
| `doEntityDrops`             | `true`  | 启用实体掉落                              |
| `doFireTick`                | `true`  | 启用火焰蔓延                              |
| `recipesUnlock`             | `true`  | 启用解锁食谱                              |
| `doLimitedCrafting`         | `false` | 禁用限制制作（允许所有物品制作）                    |
| `doMobLoot`                 | `true`  | 启用怪物掉落                              |
| `doMobSpawning`             | `true`  | 启用怪物生成                              |
| `doTileDrops`               | `true`  | 启用方块掉落                              |
| `doWeatherCycle`            | `true`  | 启用天气循环                              |
| `drowningDamage`            | `true`  | 启用溺水伤害                              |
| `fallDamage`                | `true`  | 启用掉落伤害                              |
| `fireDamage`                | `true`  | 启用火焰伤害                              |
| `keepInventory`             | `false` | 禁用玩家死亡后保留物品                         |
| `mobGriefing`               | `true`  | 启用怪物破坏世界（如 Creeper 爆炸、Enderman 取块等） |
| `pvp`                       | `true`  | 启用玩家与玩家之间的伤害                        |
| `showCoordinates`           | `false` | 禁用显示坐标                              |
| `locatorBar`                | `true`  | 启用定位条                               |
| `showDaysPlayed`            | `false` | 禁用显示已玩天数                            |
| `naturalRegeneration`       | `true`  | 启用自然再生（玩家血量会自动恢复）                   |
| `tntExplodes`               | `true`  | 启用 TNT 爆炸                           |
| `sendCommandFeedback`       | `true`  | 启用命令反馈                              |
| `maxCommandChainLength`     | `65535` | 设置命令链最大长度                           |
| `doInsomnia`                | `true`  | 启用失眠（不睡觉时怪物生成）                      |
| `commandBlocksEnabled`      | `true`  | 启用命令方块                              |
| `randomTickSpeed`           | `1`     | 设置随机 tick 速度                        |
| `doImmediateRespawn`        | `false` | 禁用立即重生（玩家死亡后等待重生）                   |
| `showDeathMessages`         | `true`  | 启用死亡信息显示                            |
| `functionCommandLimit`      | `10000` | 设置函数命令的最大执行次数                       |
| `spawnRadius`               | `10`    | 设置玩家重生时的半径                          |
| `showTags`                  | `true`  | 启用显示标签（玩家、怪物等标签）                    |
| `freezeDamage`              | `true`  | 启用冻结伤害（例如冰冻攻击）                      |
| `respawnBlocksExplode`      | `true`  | 启用重生点方块爆炸                           |
| `showBorderEffect`          | `true`  | 启用世界边界效果（例如游戏边界）                    |
| `showRecipeMessages`        | `true`  | 启用食谱信息显示                            |
| `playersSleepingPercentage` | `100`   | 设置玩家睡觉所需的百分比（100%表示所有玩家需要睡觉）        |
| `projectilesCanBreakBlocks` | `true`  | 启用投射物破坏方块（如箭矢、火球等）                  |
| `tntExplosionDropDecay`     | `false` | 禁用 TNT 爆炸后物品的掉落衰减（即掉落物品不消失）         |
